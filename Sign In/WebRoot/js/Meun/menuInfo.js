layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    $("#mname").blur(function(){
    	...
    	$.post("/EMentTet/MeunSerlvet?action=isMenuName&mname="+mname,function(res){
    	 var cs = eval('(' + res + ')'); 
    		if(res == 1){
    			layer.msg('权限名已存在');
    		}
 ...
 /*   $("#mfunction").blur(function(){
    	...
    	$.post("/MenuServlet?action=isMenuUrl&mfunction="+mfunction,function(res){
    		var cs = eval('(' + res + ')');
    		if(res == 1){
    			layer.msg('请求路径已存在');
    		}
    ...
    $("#menuAjax").click(function(){
        ...
        //ajax提交
        $.ajax({
        	url:"/EMentTet/MeunSerlvet?action=upMenu",
        	type:"post",
        	data:data2,
